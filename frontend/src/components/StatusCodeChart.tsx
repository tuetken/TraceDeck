import {
  BarChart,
  Bar,
  XAxis,
  YAxis,
  Tooltip,
  ResponsiveContainer,
  Cell,
} from 'recharts'

interface StatusCodeEntry {
  statusCode: number
  count: number
}

interface Props {
  statusCodes: StatusCodeEntry[]
}

function colorForStatus(code: number): string {
  if (code >= 500) return '#cf4b4b'  // danger — 5xx
  if (code >= 400) return '#d4a84b'  // warning — 4xx
  return '#4caf82'                    // success — 2xx/3xx
}

export default function StatusCodeChart({ statusCodes }: Props) {
  const data = [...statusCodes].sort((a, b) => a.statusCode - b.statusCode)

  return (
    <div
      className="rounded-lg p-4"
      style={{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }}
    >
      <p className="text-xs font-medium mb-4" style={{ color: 'var(--color-text-secondary)' }}>
        Status Code Distribution
      </p>
      {data.length === 0 ? (
        <div className="flex items-center justify-center h-40 text-sm" style={{ color: 'var(--color-text-muted)' }}>
          No data
        </div>
      ) : (
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={data} margin={{ top: 0, right: 8, bottom: 8, left: 0 }}>
            <XAxis
              dataKey="statusCode"
              tick={{ fill: '#8b8fa8', fontSize: 11 }}
              axisLine={false}
              tickLine={false}
            />
            <YAxis
              tick={{ fill: '#8b8fa8', fontSize: 11 }}
              axisLine={false}
              tickLine={false}
              width={40}
            />
            <Tooltip
              cursor={{ fill: 'rgba(255,255,255,0.04)' }}
              contentStyle={{
                backgroundColor: '#1e2028',
                border: '1px solid #2a2d35',
                borderRadius: 6,
                color: '#e8eaed',
                fontSize: 12,
              }}
              formatter={(value: number) => [value, 'Requests']}
            />
            <Bar dataKey="count" radius={[3, 3, 0, 0]}>
              {data.map((entry, i) => (
                <Cell key={i} fill={colorForStatus(entry.statusCode)} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
  )
}
